...
var myApp = angular.module('myApp', ['angularUtils.directives.dirPagination','angularFileUpload','ui.bootstrap']);
...
function AssociateCtrl($scope ,$http) {
  ...
  myApp.controller('HrCtrlView', ['$scope','$http', '$location', 'FileUploader', function($scope,$http, $location, FileUploader) {
    ...
    var uploader = $scope.uploader = new FileUploader({
		...
		//console.log(data);
		var url = 'hr_excel.php';
		$http.post(url, data, config).success(function (results, status, headers, config)
        {
		 console.log(results)
		 if(results =='success'){
        ...
        .error(function (results, status, headers, config)
        {
	     ...
	     $('.alert').css('display','block');
		  ...
		  $('#errormsg').html('Select file to upload');

	   }
      }
    $scope.letterabc=function(a,b,c){
	  ...
	  /* if($scope.letter ==null){
		   ...
		   jQuery('<div></div>').dialog({
					...
					title: "Alert",
					open: function () {
						var markup = '<b>Please Select request status</b>';
       ...
       $http.post("letter_autoapprove.php", data, config).success(function (results, status, headers, config)
        {
	       //console.log(data)
          if(data.cempid!=null){
               var url ="hr.php?id="+c;
			 //confirm(url);
			      jQuery('<div></div>').dialog({
					...
					title: "Alert",
					open: function () {
						var markup = '<b>Status update successfully</b>';
						jQuery(this).html(markup);
						return false;
					},
					buttons: {
	...
	$scope.checkRowData = function(icount){
		if(icount==0){
			$("#excelFile").val("");
		}
	}

//	  if(uploaderQueueLen==0){$("#excelFile").val("");}
 ...
 myApp.controller('HODCtrlView', ['$scope','$http', '$location', 'FileUploader', function($scope,$http, $location, FileUploader) {
